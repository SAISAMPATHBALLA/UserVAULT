...
def register_user(payload: RegisterRequest, db: Session):
    ...
    return {"success": True, "message": "Registration successful", "data": {"id": str(user.id), "name": user.name, "email": user.email}}
...
def login_user(payload: LoginRequest, db: Session):
    ...
    return {"success": True, "message": "Login successful", "data": {"id": str(user.id), "name": user.name, "email": user.email}}
